...
class ImageUtility:
    def trim(self, src_image, convert_to_black_white):
        ...
        If convert_to_black_white is specified, then the image is first
            processed to have better result
        """
    ...
    def split_x(self, src_image, convert_to_black_white=False):
        ...
        boxes = [(region[0], y_top, region[1], y_bottom) for region in regions if (region[1]-region[0]) > 1]  # noqa: E501
    ...
    def split_y(self, src_image, convert_to_black_white=False):
        ...
        boxes = [(x_left, region[0], x_right, region[1]) for region in regions if (region[1]-region[0]) > 1]  # noqa: E501
        return [src_image.crop(box) for box in boxes]
...
'''tk = ImageUtility()
tk.process('path to image file', 'path to output', True)
'''
